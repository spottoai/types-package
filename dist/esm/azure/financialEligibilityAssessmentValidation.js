...
import { FINANCIAL_ELIGIBILITY_ASSESSMENT_CONTRACT_VERSION_V1, FINANCIAL_ELIGIBILITY_ASSESSMENT_SCHEMA_VERSION_V1, } from './financialSavingsAuthority.js';
import { createFinancialSavingsDenominatorIdV1 } from './financialSavingsCoordinateValidation.js';
import { isFinancialScopeBaselineEnvelopeV2 } from './financialScopeBaselineValidation.js';
import { canonicalizeFinancialSavingsJsonValue, hasExactFinancialSavingsFields, haveSameFinancialSavingsSet, isFinancialSavingsHash, isFinancialSavingsIdentity, isFinancialSavingsIsoInstant, isFinancialSavingsRecord, } from './financialSavingsAuthorityValidationPrimitives.js';
...
export const canonicalizeFinancialEligibilityAssessmentIdentityV1 = (value) => JSON.stringify(canonicalizeFinancialSavingsJsonValue(value.status === 'unavailable'
...
export const createFinancialEligibilityAssessmentIdV1 = (value) => `sha256:${sha256Utf8(canonicalizeFinancialEligibilityAssessmentIdentityV1(value))}`;
export const validateFinancialEligibilityBaselineV1 = (value, context) => {
    if (!isFinancialScopeBaselineEnvelopeV2(value) || value.status !== 'available' || value.baselineKind !== 'owner')
        return false;
    if (value.scopeKind === 'subscription-residual')
        return false;
    const assessment = context.assessmentById.get(value.assessmentId);
    return (haveSameFinancialSavingsSet(value.providerAccountRefs, context.authority.providerAccountRefs) &&
...
export const validateFinancialEligibilityAssessmentV1 = (value, context, baselineById) => {
    ...
    if (value.status === 'unavailable') {
        ...
        return value.eligibilityId === createFinancialEligibilityAssessmentIdV1(identity);
    }
    ...
    if (!baseline || baseline.scopeId !== value.scopeId || !isFinancialSavingsRecord(value.denominator))
        return false;
    ...
    if (ruleEvidence?.role !== 'eligibility-rule' ||
        ...
        return false;
    if (!hasExactFinancialSavingsFields(value.denominator, ['denominatorId', 'kind', 'componentIds', 'amount', 'currencyCode']) ||
        ...
        !haveSameFinancialSavingsSet([...value.eligibleComponentIds, ...value.excludedComponentIds], baseline.components.map(component => component.componentId)) ||
    ...
    try {
        const eligibleAmount = formatExactDecimalValue(sumCanonicalDecimals(value.eligibleComponentIds.map(componentId => baselineComponentById.get(componentId).amount)));
    ...
    for (const mapping of value.currentBaselineMapping.mappings) {
        if (!isFinancialSavingsRecord(mapping) ||
