import { PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1, PORTAL_ACTIVITY_ANALYSIS_RESPONSE_SCHEMA_VERSION, } from './activityLogAnalysis.js';
import { CONFIDENCES, EFFECTS, LIMITATIONS, MONTH, ORIGINS, POWER_SUFFICIENCY, TAG_IDS, countRowsTotal, hasExactKeys, hasValidTimes, isChronologicalCountRows, isCount, isCountRows, isDate, isEvidenceIds, isGroupIds, isPortalActivityAnalysisGroupId, isPortalActivityEvidenceId, isPortalActivityLogClassification, isPositiveCount, isReasons, isRecord, isScope, isSortedUniqueStrings, isText, isTimestamp, isTags, utf8ByteLength, } from './activityLogAnalysisPublicValidationHelpers.js';
...
const isSeries = (value, subscriptionId, resourceId, requestedMonths) => {
    ...
    for (let index = 0; index < value.dailyCounts.length; index += 1) {
        ...
        if (!hasExactKeys(row, ['date', 'count']) ||
            !isDate(row.date) ||
...
const isResource = (value, subscriptionId, resourceId) => {
    if (!hasExactKeys(value, [
        ...
        !isPositiveCount(value.eventCount) ||
        !hasValidTimes(value) ||
        !isCountRows(value.operationCounts, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.operationCountsPerItem, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.operationCodeUnits) ||
        ...
        !CONFIDENCES.has(value.confidence))
        return false;
    return (countRowsTotal(value.operationCounts) === value.eventCount &&
...
const isDerivedBase = (value, subscriptionId, resourceId) => isScope(value.scope, subscriptionId, resourceId) &&
    isPositiveCount(value.eventCount) &&
    hasValidTimes(value) &&
    typeof value.confidence === 'string' &&
    CONFIDENCES.has(value.confidence) &&
...
const isDerived = (value, subscriptionId, resourceId) => {
    if (!isRecord(value) || typeof value.derivedType !== 'string')
        return false;
    if (value.derivedType === 'operationSummary') {
        return (hasExactKeys(value, ['groupId', 'derivedType', ...DERIVED_BASE_KEYS, 'operation', 'operationEffect', 'executionOrigin', 'distinctResourceCount', 'resultCounts'], ['provider', 'resourceType']) &&
    ...
    if (value.derivedType === 'securitySensitiveActivity') {
        return (hasExactKeys(value, ['groupId', 'derivedType', ...DERIVED_BASE_KEYS, 'capabilityTag', 'operation', 'executionOrigin', 'resultCounts']) &&
            isPortalActivityAnalysisGroupId(value.groupId) &&
            isDerivedBase(value, subscriptionId, resourceId) &&
...
const isCollection = (value, maximum, validator) => hasExactKeys(value, ['totalCount', 'returnedCount', 'truncated', 'items']) &&
...
const isFacets = (value) => {
    if (!hasExactKeys(value, ['tags', 'operations', 'providers', 'resourceTypes', 'results', 'operationEffects', 'executionOrigins']))
        return false;
    const facet = (candidate, maximum, textMaximum, allowed) => isCollection(candidate, maximum, (row) => isCountRows([row], 1, textMaximum, allowed));
    return (facet(value.tags, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.facetTagRows, 128, TAG_IDS) &&
...
const isMonthList = (value) => isSortedUniqueStrings(value, 1200, month => MONTH.test(month));
...
export const isPortalActivityAnalysisResponse = (value) => {
    if (!hasExactKeys(value, [
        'schemaVersion',
        'generatedAt',
        'subscriptionId',
        ...
        !isText(value.subscriptionId, 2048) ||
    ...
    if (!isCollection(value.activitySeries, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.activitySeries, (item) => isSeries(item, subscriptionId, resourceId, available)) ||
        !isCollection(value.resources, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.resourceSummaries, (item) => isResource(item, subscriptionId, resourceId)) ||
        !isCollection(value.operationSummaries, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.operationSummaries, (item) => isDerived(item, subscriptionId, resourceId) && item.derivedType === 'operationSummary') ||
        !isCollection(value.securitySensitive, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.securitySensitive, (item) => isDerived(item, subscriptionId, resourceId) && item.derivedType === 'securitySensitiveActivity') ||
        !isCollection(value.powerPatterns, PORTAL_ACTIVITY_ANALYSIS_LIMITS_V1.powerPatterns, (item) => isDerived(item, subscriptionId, resourceId) && item.derivedType === 'powerPatternEvidence'))
        return false;
    ...
    if (new Set(publicIds).size !== publicIds.length)
        return false;
    if (!isSortedUniqueStrings(value.limitations, LIMITATIONS.size, item => LIMITATIONS.has(item)))
        return false;
...
export function assertPortalActivityAnalysisResponse(value) {
    if (!isPortalActivityAnalysisResponse(value))
