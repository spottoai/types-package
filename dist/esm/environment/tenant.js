...
import { ENVIRONMENT_RUN_ID_PATTERN, SHA256_PATTERN, hasExactKeys, hasSafeContainerShape, isBoundedString, isCanonicalUtcTimestamp, isNonNegativeInteger, isRecord, isSafeLabel, isScopeIdentifier, isSourceIdentity, utf8ByteLength, } from './internal.js';
...
export const buildEnvironmentTenantScopeQualifiedSubjectV1 = (scope) => {
    if (!isEnvironmentTenantScopeV1(scope))
...
export const isEnvironmentTenantScopeV1 = (value) => isRecord(value) && hasExactKeys(value, ['kind', 'tenantId']) && value.kind === 'azure-tenant' && isScopeIdentifier(value.tenantId);
/** Validates the completed tenant-sync generation bound to a tenant environment generation. */
export const isEnvironmentTenantSourceBindingV1 = (value) => isRecord(value) &&
...
const isReferenceArray = (value) => Array.isArray(value) && value.length <= ENVIRONMENT_CONTRACT_LIMITS_V1.boundedListItems && value.every(isEnvironmentLogicalEvidenceReferenceV1);
const isBoundedList = (value, itemGuard) => isRecord(value) &&
...
const isCountRecord = (value, keys) => isRecord(value) && hasExactKeys(value, keys) && keys.every(key => isNonNegativeInteger(value[key]));
const isGlobalAdministrator = (value) => isRecord(value) &&
...
const isGovernanceFinding = (value) => isRecord(value) &&
    ...
    isReferenceArray(value.sourceReferences);
const isCommitment = (value) => isRecord(value) &&
...
export const isEnvironmentTenantProjectionV1 = (value) => {
...
const descriptorLimit = (name) => name === 'projection.json' ? ENVIRONMENT_CONTRACT_LIMITS_V1.projectionBytes : ENVIRONMENT_CONTRACT_LIMITS_V1.environmentIndexBytes;
/** Validates one tenant environment document descriptor. */
export const isEnvironmentTenantDocumentDescriptorV1 = (value) => isRecord(value) &&
    hasExactKeys(value, ['name', 'mediaType', 'byteCount', 'contentSha256', 'approximateTokenCount']) &&
    typeof value.name === 'string' &&
    ENVIRONMENT_TENANT_DOCUMENT_NAMES_V1.includes(value.name) &&
...
export const isEnvironmentTenantDocumentDescriptorSetV1 = (value) => Array.isArray(value) &&
...
export const buildEnvironmentTenantTreeDigestPreimageV1 = (descriptors) => {
    if (!isEnvironmentTenantDocumentDescriptorSetV1(descriptors))
...
export const isEnvironmentTenantCompiledGenerationPointerV1 = (value) => hasSafeContainerShape(value) &&
    isRecord(value) &&
...
export const isEnvironmentTenantLogicalReferenceV1 = (value) => typeof value === 'string' && REFERENCE_PATTERN.test(value) && isEnvironmentLogicalEvidenceReferenceV1(value);
