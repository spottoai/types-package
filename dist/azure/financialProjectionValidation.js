...
exports.isFinancialProjectionEnvelopeV1 = exports.canonicalizeFinancialProjectionIdentityV1 = void 0;
...
const UNAVAILABLE = new Set(financialProjection_1.FINANCIAL_PROJECTION_UNAVAILABLE_REASONS_V1);
...
const isDecimal = (value, currency = 'AUD') => typeof value === 'string' && (0, financialValidationPrimitives_1.isCanonicalExactMoney)({ amount: value, currencyCode: currency });
...
const isTargetPeriodProfileCompatible = (convention, profile, available) => convention === 'normalized-average-month'
    ...
    : convention === 'same-period-quantity'
        ...
        : profile === undefined;
const equalsDecimal = (left, right) => (0, exactDecimal_1.formatExactDecimalValue)(left) === (0, exactDecimal_1.formatExactDecimalValue)(right);
const roundRationalHalfEven = (numerator, denominator, targetScale) => {
    ...
    return (0, exactDecimal_1.formatExactDecimalValue)({ coefficient: rounded, scale: targetScale });
};
const isCommitmentCoverage = (value, targetAmount, currency, profile) => {
    if (!isRecord(value) ||
        ...
        (0, exactDecimal_1.parseCanonicalDecimal)(String(value.purchaseQuantity)).coefficient <= 0n ||
    ...
    if (quantities.some(quantity => !hasExactFields(quantity, ['amount', 'unit']) || !isNonNegativeDecimal(quantity.amount) || !isUnit(quantity.unit)) ||
        ...
        return false;
    const eligible = (0, exactDecimal_1.parseCanonicalDecimal)(String(value.eligibleQuantity.amount));
    const allocated = (0, exactDecimal_1.sumCanonicalDecimals)([
    ...
    if (!equalsDecimal(eligible, allocated))
        return false;
    const uncoveredCharge = (0, exactDecimal_1.multiplyExactDecimalValues)((0, exactDecimal_1.parseCanonicalDecimal)(String(value.uncoveredQuantity.amount)), (0, exactDecimal_1.parseCanonicalDecimal)(String(value.uncoveredRate.amount)));
    const replayedTarget = (0, exactDecimal_1.addExactDecimalValues)((0, exactDecimal_1.parseCanonicalDecimal)(String(value.commitmentCharge.amount)), uncoveredCharge);
    if (!equalsDecimal(replayedTarget, (0, exactDecimal_1.parseCanonicalDecimal)(targetAmount)))
        return false;
    if (!hasExactFields(value.quote, ['kind', 'amount', 'currencyCode', 'termMonths', 'termDayCount']) ||
        ...
        return false;
    const purchaseQuantity = (0, exactDecimal_1.parseCanonicalDecimal)(String(value.purchaseQuantity));
    if (purchaseQuantity.scale !== 0)
        return false;
    const quoteAmount = (0, exactDecimal_1.parseCanonicalDecimal)(String(value.quote.amount));
    const commitmentCharge = (0, exactDecimal_1.parseCanonicalDecimal)(String(value.commitmentCharge.amount));
    ...
    if (!capacityHours ||
        (0, exactDecimal_1.subtractExactDecimalValues)((0, exactDecimal_1.parseCanonicalDecimal)(String(value.coveredQuantity.amount)), (0, exactDecimal_1.multiplyExactDecimalValues)(purchaseQuantity, (0, exactDecimal_1.parseCanonicalDecimal)(capacityHours))).coefficient > 0n)
        return false;
    if (isNormalizedAverageMonthProfile(profile)) {
        return equalsDecimal((0, exactDecimal_1.multiplyExactDecimalValues)(quoteAmount, purchaseQuantity), (0, exactDecimal_1.multiplyExactDecimalValues)(commitmentCharge, (0, exactDecimal_1.parseCanonicalDecimal)(String(value.quote.termMonths))));
    }
    if (!isObservedPeriodProfile(profile))
        return false;
    const allocatedQuote = (0, exactDecimal_1.multiplyExactDecimalValues)((0, exactDecimal_1.multiplyExactDecimalValues)(quoteAmount, purchaseQuantity), (0, exactDecimal_1.parseCanonicalDecimal)(String(profile.dayCount)));
    return (roundRationalHalfEven(allocatedQuote, BigInt(Number(value.quote.termDayCount)), Number(profile.currencyMinorUnitScale)) ===
        (0, exactDecimal_1.formatExactDecimalValue)(commitmentCharge));
};
...
const isProjectionIdentity = (value) => isRecord(value) &&
    ...
    value.schemaVersion === financialProjection_1.FINANCIAL_PROJECTION_SCHEMA_VERSION_V1 &&
    value.contractVersion === financialProjection_1.FINANCIAL_PROJECTION_CONTRACT_VERSION_V1 &&
...
const canonicalizeFinancialProjectionIdentityV1 = (value) => {
...
exports.canonicalizeFinancialProjectionIdentityV1 = canonicalizeFinancialProjectionIdentityV1;
const isAmounts = (value, currency) => isRecord(value) &&
    ...
    (0, exactDecimal_1.formatExactDecimalValue)((0, exactDecimal_1.sumCanonicalDecimals)([String(value.affected), String(value.unchanged)])) === value.total;
const isAvailable = (value) => {
    ...
    if ((0, exactDecimal_1.formatExactDecimalValue)((0, exactDecimal_1.sumCanonicalDecimals)(appliedTargets.map(target => target.targetAmount))) !==
        value.target.affected)
        return false;
    const current = value.current;
    const target = value.target;
    const change = value.change;
    const delta = (0, exactDecimal_1.subtractExactDecimalValues)((0, exactDecimal_1.parseCanonicalDecimal)(target.total), (0, exactDecimal_1.parseCanonicalDecimal)(current.total));
    const expectedDelta = (0, exactDecimal_1.formatExactDecimalValue)(delta);
    const expectedSavings = delta.coefficient < 0n ? (0, exactDecimal_1.formatExactDecimalValue)({ coefficient: -delta.coefficient, scale: delta.scale }) : '0';
    ...
    return isProjectionIdentity(identity) && value.projectionId === `sha256:${(0, sha256_1.sha256Utf8)((0, exports.canonicalizeFinancialProjectionIdentityV1)(identity))}`;
};
...
const isFinancialProjectionEnvelopeV1 = (value) => isRecord(value) && (isAvailable(value) || isUnavailable(value));
exports.isFinancialProjectionEnvelopeV1 = isFinancialProjectionEnvelopeV1;
//# sourceMappingURL=financialProjectionValidation.js.map