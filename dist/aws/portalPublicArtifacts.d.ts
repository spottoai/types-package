...
export declare const AWS_PORTAL_ACCOUNT_SUMMARY_AI_COST_SUMMARY_LOGICAL_NAME: "account-summary-ai-cost-summary.json.gz";
...
export declare function buildAwsPortalResourceCollectionHistoryIdentityKey(scope: AwsPortalResourceCollectionScope, generatedAt: string): string;
export declare function buildAwsPortalResourceCollectionHistoryLogicalNameForScope(scope: AwsPortalResourceCollectionScope, generatedAt: string): AwsPortalResourceCollectionHistoryLogicalName;
export declare function buildAwsPortalAccountSummaryHistoryIdentityKey(scope: Omit<AwsPortalAccountSummaryScope, 'comparisonBillingPeriod' | 'metricTimeWindow'>): string;
export declare function buildAwsPortalAccountSummaryHistoryLogicalNameForScope(scope: Omit<AwsPortalAccountSummaryScope, 'comparisonBillingPeriod' | 'metricTimeWindow'>): AwsPortalAccountSummaryHistoryLogicalName;
export declare function buildAwsPortalAccountSummaryTargetKey(scope: AwsPortalAccountSummaryScope): string;
export declare function buildAwsPortalAccountSummaryAiCostSummaryTargetKey(scope: AwsPortalAccountSummaryScope, audience: AwsPortalAiAudience): string;
export type AwsPortalDiscoveryFamily = 'ec2-instance' | 'ebs-volume' | 'efs-file-system' | 'lambda-function' | 'elasticache-cache-cluster' | 'elasticache-serverless-cache' | 'vpc' | 'subnet' | 'route-table' | 'internet-gateway' | 'nat-gateway' | 'network-interface' | 'virtual-private-gateway' | 'load-balancer-v2' | 'classic-load-balancer' | 'security-group' | 'rds-db-cluster' | 'rds-db-instance';
export type AwsPortalMetricFamily = 'ec2-core-utilization' | 'ebs-volume-performance' | 'lambda-function-performance' | 'load-balancer-v2-network-traffic' | 'load-balancer-v2-application-traffic' | 'rds-db-instance-utilization';
export type AwsPortalRecommendationSource = 'custom' | 'compute-optimizer' | 'cost-optimization-hub' | 'resilience-hub' | 'security-hub' | 'trusted-advisor' | 'well-architected';
...
export interface AwsPortalRecommendationCounts {
    byLifecycleStatus: Array<{
        lifecycleStatus: string;
        count: number;
    }>;
    bySourcePresence: Array<{
...
export interface AwsPortalResourceCollectionBody<AccountId extends string = string> {
    ...
    coverage: {
        ...
        recommendations: {
            sources: AwsPortalRecommendationCoverage<AccountId>[];
        };
        virtualTags?: AwsPortalVirtualTagEvidence;
    };
    resources: AwsPortalResourceDetailItem[];
}
type AwsPortalQualifiedEnvelope<ArtifactType extends 'resource-collection' | 'account-summary', AccountId extends string, RunId extends string> = AwsPublicArtifactEnvelope<ArtifactType, AccountId, RunId> & {
    portalSchemaVersion: typeof AWS_PORTAL_PUBLIC_ARTIFACT_SCHEMA_VERSION;
    logicalName: ArtifactType extends 'resource-collection' ? typeof AWS_PORTAL_RESOURCE_COLLECTION_LOGICAL_NAME : typeof AWS_PORTAL_ACCOUNT_SUMMARY_LOGICAL_NAME;
};
export type AwsPortalResourceCollectionDetailArtifact<AccountId extends string = string, RunId extends string = string> = AwsPortalQualifiedEnvelope<'resource-collection', AccountId, RunId> & AwsPortalResourceCollectionBody<AccountId>;
...
export type AwsPortalAccountSummaryDetailArtifact<AccountId extends string = string, RunId extends string = string> = AwsPortalQualifiedEnvelope<'account-summary', AccountId, RunId> & AwsPortalAccountSummaryBodyV2<AccountId>;
export {};
//# sourceMappingURL=portalPublicArtifacts.d.ts.map