...
export function validateAwsPortalBillingBlock(
  ...
  assertExactKeys(
    billing,
    ['scope', 'freshness', 'summary', 'totalsByCurrency', ...windowFields, ...(accountBody ? accountFields : resourceFields)],
    field
  );
 ...
 */
function validateBillingEvidenceWindow(billing: Record<string, unknown>, outerBilling: AwsPortalBillingScope, field: string): void {
  ...
  if (billing.windowCoverage !== undefined && !hasWindow) throw new Error(`${field}.windowCoverage requires ${field}.window.`);
  if (!hasWindow) return;
  ...
  if (shiftUtcDate(endDateExclusive, -30) !== startDateInclusive) throw new Error(`${field}.window must cover exactly 30 UTC days.`);
  ...
  if (billing.contributingBillingPeriods === undefined) return;
  if (!Array.isArray(billing.contributingBillingPeriods)) throw new Error(`${field}.contributingBillingPeriods must be an array.`);
  const periods = billing.contributingBillingPeriods.map((entry, index) => {
    ...
    if (end < start) throw new Error(`${itemField}.end must not precede ${itemField}.start.`);
    return { start, end };
  });
  assertUnique(
    periods.map(period => `${period.start}|${period.end}`),
    `${field}.contributingBillingPeriods`
  );
