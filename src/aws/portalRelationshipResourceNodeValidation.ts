import { AWS_PORTAL_RELATIONSHIP_DISCOVERY_FAMILIES, type AwsPortalRelationshipDiscoveryFamily } from './portalRelationshipPublicArtifacts';
import { validateResourceBillingEvidence } from './portalPublicArtifactNestedEvidenceValidation';
import {
  asRecord,
  assertExactKeys,
  assertValue,
...
import { assertAccount, assertRequiredKeys } from './pluginPublicArtifactValidationHelpers';
const RESOURCE_COMMON_KEYS = ['provider', 'accountId', 'family', 'resourceRegion', 'stableKey', 'resourceType', 'tags', 'billing'] as const;
...
} as const satisfies Record<AwsPortalRelationshipDiscoveryFamily, readonly string[]>;
...
export function validateAwsPortalRelationshipResourceNode(
  ...
  if (!regions.has(resourceRegion)) throw new Error(`${field}.resourceRegion is outside artifact scope.`);
  ...
  assertRequiredKeys(
    data,
    ['provider', 'accountId', 'family', 'resourceRegion', 'stableKey', 'resourceType', ...REQUIRED_RESOURCE_KEYS[family]],
    field
  );
  for (const key of Object.keys(data)) {
    if (STRING_FIELDS.has(key) && data[key] !== undefined) requiredString(data[key], `${field}.${key}`);
    if (NUMBER_FIELDS.has(key) && data[key] !== undefined) finiteNumber(data[key], `${field}.${key}`);
    if (BOOLEAN_FIELDS.has(key) && data[key] !== undefined) requiredBoolean(data[key], `${field}.${key}`);
    if (STRING_ARRAY_FIELDS.has(key) && data[key] !== undefined) stringArray(data[key], `${field}.${key}`);
  }
  if (data.tags !== undefined) validateTags(data.tags, `${field}.tags`);
  if (data.billing !== undefined) validateResourceBillingEvidence(data.billing, `${field}.billing`);
  validateNativeScopeIdentity(data, accountId, resourceRegion, field);
  validateNestedResourceArrays(data, family, field);
}

function validateNativeScopeIdentity(data: Record<string, unknown>, accountId: string, resourceRegion: string, field: string): void {
  if (data.ownerId !== undefined) assertAccount(data.ownerId, accountId, `${field}.ownerId`);

  Object.entries(data).forEach(([key, value]) => {
    if (typeof value !== 'string') return;
    if (key.endsWith('Arn') || key === 'stableKey' || key === 'kmsKeyId') {
      if (value.startsWith('arn:')) validateScopedArn(value, accountId, resourceRegion, `${field}.${key}`);
    }
  ...
  for (const key of ['availabilityZones', 'availabilityZoneNames']) {
    ...
    data[key].forEach((value, index) => {
      if (typeof value === 'string') validateAvailabilityZone(value, resourceRegion, `${field}.${key}[${index}]`);
    });
  }
}

function validateScopedArn(value: string, accountId: string, resourceRegion: string, field: string): void {
  const match = /^arn:(aws(?:-[a-z0-9-]+)?):[^:]+:([^:]*):([^:]*):.+$/.exec(value);
  if (!match) throw new Error(`${field} must be a canonical AWS ARN.`);
  const [, , arnRegion, arnAccountId] = match;
  if (arnRegion !== resourceRegion) throw new Error(`${field} ARN Region must match its resource node.`);
  if (arnAccountId !== accountId) throw new Error(`${field} ARN account must match its resource node.`);
}

function validateAvailabilityZone(value: string, resourceRegion: string, field: string): void {
...
function validateNestedResourceArrays(data: Record<string, unknown>, family: AwsPortalRelationshipDiscoveryFamily, field: string): void {
  if (family === 'vpc') {
    validateObjectArray(data.cidrBlockAssociations, ['associationId', 'cidrBlock', 'state'], ['cidrBlock'], `${field}.cidrBlockAssociations`);
    validateObjectArray(
      data.ipv6CidrBlockAssociations,
      ['associationId', 'ipv6CidrBlock', 'state'],
      ['ipv6CidrBlock'],
      `${field}.ipv6CidrBlockAssociations`
    );
    ...
    validateObjectArray(
      data.associations,
      ['associationId', 'subnetId', 'gatewayId', 'main', 'associationState', 'associationStateMessage'],
      [],
      `${field}.associations`
    );
    ...
    validateObjectArray(
      data.addresses,
      ['allocationId', 'associationId', 'isPrimary', 'networkInterfaceId', 'privateIp', 'publicIp', 'status'],
      [],
      `${field}.addresses`
    );
    ...
    validateObjectArray(
      ...
      ['dbInstanceIdentifier'],
      `${field}.dbClusterMembers`
    );
  }
}

function validateObjectArray(value: unknown, keys: readonly string[], required: readonly string[], field: string): void {
  asArray(value, field).forEach((entry, index) => {
    ...
    Object.entries(item).forEach(([key, itemValue]) => {
      ...
      if (['main', 'deleteOnTermination', 'isPrimary', 'isClusterWriter'].includes(key)) requiredBoolean(itemValue, `${itemField}.${key}`);
      else if (key === 'promotionTier') finiteNumber(itemValue, `${itemField}.${key}`);
      else requiredString(itemValue, `${itemField}.${key}`);
    });
  });
}
