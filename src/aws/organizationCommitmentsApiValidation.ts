import {
  AWS_ORGANIZATION_COMMITMENTS_ISSUE_CODES,
  AWS_ORGANIZATION_COMMITMENTS_REFRESH_STAGES,
  AWS_ORGANIZATION_COMMITMENTS_REFRESH_STAGE_STATUSES,
  AWS_ORGANIZATION_COMMITMENTS_REFRESH_STATES,
  AWS_ORGANIZATION_COMMITMENTS_SCHEMA_VERSION,
  AWS_ORGANIZATION_COMMITMENTS_SCOPE_UNAVAILABLE_REASONS,
...
export function validateAwsOrganizationCommitmentsRefreshAcceptedResponse(
  value: unknown,
  expected: AwsOrganizationCommitmentsAdmissionExpectedIdentity
): AwsOrganizationCommitmentsRefreshAcceptedResponse {
  const response = asRecord(value, 'organizationCommitmentsAdmission');
  const disposition = requiredEnum(
    response.disposition,
    ['accepted', 'already-running', 'cooldown'] as const,
    'organizationCommitmentsAdmission.disposition'
  );
  assertExactKeys(
    response,
    [
...
export function validateAwsOrganizationCommitmentsScopeListResponse(
  value: unknown,
  expectedCompanyId: string
): AwsOrganizationCommitmentsScopeListResponse {
  ...
  if (!Array.isArray(response.organizations)) throw new Error('organizationCommitmentsScopes.organizations must be an array.');
  ...
  response.organizations.forEach((entry, index) => {
    ...
    assertExactKeys(
      scope,
      ['scopeType', 'estateId', 'name', 'organizationId', 'managementAccountId', 'accountCount', 'availability', 'canView', 'canRefresh', 'reason'],
      field
    );
    ...
    if (accountCount < 1) throw new Error(`${field}.accountCount must include the management account.`);
    const availability = requiredEnum(scope.availability, ['available', 'unavailable'] as const, `${field}.availability`);
    requiredBoolean(scope.canView, `${field}.canView`);
    const canRefresh = requiredBoolean(scope.canRefresh, `${field}.canRefresh`);
    if (availability === 'available') {
      if (scope.reason !== undefined) throw new Error(`${field}.reason is not allowed when the scope is available.`);
    } else {
      requiredEnum(scope.reason, AWS_ORGANIZATION_COMMITMENTS_SCOPE_UNAVAILABLE_REASONS, `${field}.reason`);
      if (canRefresh) throw new Error(`${field}.canRefresh must be false when the scope is unavailable.`);
    }
  });
...
export function validateAwsOrganizationCommitmentsRefreshStatusResponse(
  value: unknown,
  expected: AwsOrganizationCommitmentsStatusExpectedIdentity
): AwsOrganizationCommitmentsRefreshStatusResponse {
  ...
  assertValue(
    response.targetManifestRevision,
    requiredString(expected.targetManifestRevision, 'expected.targetManifestRevision'),
    'organizationCommitmentsStatus.targetManifestRevision'
  );
  ...
  if (state === 'not-started') {
    for (const key of ['requestId', 'correlationId', 'requestedAt', 'failureCode', 'retryable'] as const) {
      if (response[key] !== undefined) throw new Error(`organizationCommitmentsStatus.${key} is not allowed before a refresh starts.`);
    }
  } else {
    const requestId = requiredString(response.requestId, 'organizationCommitmentsStatus.requestId');
    if (expected.requestId !== undefined) assertValue(requestId, expected.requestId, 'organizationCommitmentsStatus.requestId');
  ...
  if (response.retryable !== undefined) requiredBoolean(response.retryable, 'organizationCommitmentsStatus.retryable');
  if (response.nextEligibleAt !== undefined) isoTimestamp(response.nextEligibleAt, 'organizationCommitmentsStatus.nextEligibleAt');
  validateStages(response.stages);
  if (response.latestArtifact !== undefined) validateLatestArtifact(response.latestArtifact, state);
...
function validateStages(value: unknown): void {
  ...
  value.forEach((entry, index) => {
    ...
    if (stage.completedCount !== undefined) {
      ...
      if (completed > total) throw new Error(`${field}.completedCount must not exceed totalCount.`);
    }
    if (stage.failureCode !== undefined) requiredEnum(stage.failureCode, AWS_ORGANIZATION_COMMITMENTS_ISSUE_CODES, `${field}.failureCode`);
    if (status === 'failed' && stage.failureCode === undefined) throw new Error(`${field}.failureCode is required when the stage failed.`);
    if (stage.retryable !== undefined) requiredBoolean(stage.retryable, `${field}.retryable`);
    isoTimestamp(stage.updatedAt, `${field}.updatedAt`);
  });
}

function validateLatestArtifact(value: unknown, refreshState: string): void {
  ...
  const state = requiredEnum(artifact.state, ['available', 'stale', 'unavailable'] as const, 'organizationCommitmentsStatus.latestArtifact.state');
...
function validateOrganizationApiIdentity(
  value: Record<string, unknown>,
  expected: { companyId: string; estateId: string; organizationId?: string; managementAccountId?: string },
  field: string
): void {
  ...
  if (expected.organizationId !== undefined) assertValue(organizationId, expected.organizationId, `${field}.organizationId`);
...
function assertUniqueSorted(values: readonly string[], field: string): void {
  ...
  if (JSON.stringify(values) !== JSON.stringify(sorted)) throw new Error(`${field} must be sorted.`);
}
