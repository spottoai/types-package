import {
  ...
  type ResourceSchedulePermissionChangeOperationProjection,
  type ResourceSchedulePermissionManifestConsent,
  type ResourceSchedulePermissionManifestProjection,
  type ResourceSchedulePermissionOperationProjection,
...
export function isResourceSchedulePermissionManifestProjection(value: unknown): value is ResourceSchedulePermissionManifestProjection {
  if (
  ...
  if (
    addedKeys.size !== change.addedOperations.length ||
  ...
  if (
    (value.change.status === 'unchanged' && (change.addedOperations.length > 0 || change.removedOperations.length > 0)) ||
  ...
  const validGrantGroups = grantGroups.every(group => {
    ...
    const actions = group.actions as ResourceSchedulePermissionOperationProjection[];
    const dataActions = group.dataActions as ResourceSchedulePermissionOperationProjection[];
  ...
  if (
    !validGrantGroups ||
    !value.orderedGrantGroupHashes.every(
      (hash, index) => isBoundedString(hash, 200) && isRecord(grantGroups[index]) && hash === grantGroups[index].groupHash
    )
  ) {
    return false;
  }
  const currentOperationKeys = new Set<string>();
  for (const group of grantGroups) {
    if (!isRecord(group)) return false;
    for (const operation of group.actions as ResourceSchedulePermissionOperationProjection[]) {
      currentOperationKeys.add(
        permissionChangeOperationKey({
    ...
    for (const operation of group.dataActions as ResourceSchedulePermissionOperationProjection[]) {
      currentOperationKeys.add(
        permissionChangeOperationKey({
          groupHash: String(group.groupHash),
          providerScopeId: String(group.providerScopeId),
  ...
  return (
    change.addedOperations.every(operation => currentOperationKeys.has(permissionChangeOperationKey(operation))) &&
    change.removedOperations.every(operation => !currentOperationKeys.has(permissionChangeOperationKey(operation)))
  );
}
function permissionChangeOperationKey(value: ResourceSchedulePermissionChangeOperationProjection): string {
  return [value.groupHash, value.providerScopeId, value.exactAssignmentScope, value.operationKind, value.operation]
    .map(item => item.toLowerCase())
    .join('\u0000');
}
function isPermissionChangeOperations(value: unknown): value is ResourceSchedulePermissionChangeOperationProjection[] {
  return (
    Array.isArray(value) &&
...
function isPermissionOperations(value: unknown): value is ResourceSchedulePermissionOperationProjection[] {
  if (!Array.isArray(value) || value.length > RESOURCE_STRATEGY_CONTRACT_LIMITS.metadataItems) return false;
...
export function isResourceSchedulePermissionManifestConsent(value: unknown): value is ResourceSchedulePermissionManifestConsent {
  return (
    isWithinJsonByteLimit(value, RESOURCE_STRATEGY_CONTRACT_LIMITS.publicDtoBytes) &&
    isRecord(value) &&
