...
const MAX_WRITE_REQUEST_BYTES = 32 * 1_024;
const MAX_PUBLIC_DTO_BYTES = 16 * 1_024;
...
function hasExactFields<RequiredField extends string, OptionalField extends string = never>(
  value: unknown,
  requiredFields: readonly RequiredField[],
  optionalFields: readonly OptionalField[] = []
): value is JsonRecord & Record<RequiredField, unknown> & Partial<Record<OptionalField, unknown>> {
...
function isWithinJsonSize(value: unknown, maximumBytes: number): boolean {
  try {
...
function localTimeToMinutes(value: unknown): number | undefined {
...
function isUniqueBoundedStringArray(value: unknown, minimumLength: number, maximumLength: number): value is string[] {
  if (
    !Array.isArray(value) ||
...
function isAvailabilityConfiguration(value: unknown): value is BastionAvailabilityWeeklyConfiguration {
  ...
  for (const window of windows) {
    if (
      !hasExactFields(window, ['windowId', 'daysOfWeek', 'accessStartTimeLocal', 'accessEndTimeLocal']) ||
  ...
  return !intervals.some((left, leftIndex) =>
    intervals.some((right, rightIndex) => {
...
export function isBastionScheduleRun(value: unknown): value is BastionScheduleRun {
  ...
  return (
    isBoundedString(value.scheduleId, MAX_ID_LENGTH) &&
...
export function isBastionScheduleReadinessV1(value: unknown): value is BastionScheduleReadinessV1 {
  if (
    ...
    !hasExactFields(
      value,
      ['schemaVersion', 'status', 'companyId', 'cloudAccountId', 'subscriptionId', 'resourceId', 'requiredActions', 'missingActions', 'checkedAtUtc'],
      ['reasonCode']
    )
  ) {
    return false;
