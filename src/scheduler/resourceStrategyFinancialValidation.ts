import {
  ...
  type ResourceScheduleMoneyProjection,
  type ResourceSchedulePreviewRequest,
  type ResourceSchedulePreviewResponse,
  type ResourceSchedulePreviewUnavailableReason,
...
function isMoneyProjection(value: unknown): value is ResourceScheduleMoneyProjection {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, [
      'measure',
  ...
  const toScaledInteger = (amount: string): bigint => {
    const [whole, fraction = ''] = amount.split('.');
    return BigInt(whole) * decimalScale + BigInt(fraction.padEnd(8, '0'));
  };
  const componentTotal = value.components.reduce((total, component) => total + toScaledInteger((component as { amount: string }).amount), 0n);
  if (componentTotal !== toScaledInteger(value.amount)) return false;
  return (
...
function isScenarioMoneyProjection(value: unknown): value is ResourceScheduleScenarioMoneyProjection {
  return isMoneyProjection(value) && value.additivity === 'scenario';
}
export function isResourceSchedulePreviewRequest(value: unknown): value is ResourceSchedulePreviewRequest {
  return (
...
export function isResourceSchedulePreviewResponse(value: unknown): value is ResourceSchedulePreviewResponse {
  if (
    !isWithinJsonByteLimit(value, RESOURCE_STRATEGY_CONTRACT_LIMITS.publicDtoBytes) ||
    !isRecord(value) ||
  ...
  const validWindows = value.windows.every(
    window =>
      isRecord(window) &&
  ...
  return (
    hasOnlyKeys(value, ['draftHash', 'requestVersion', 'availability', 'windows', 'unavailableReason']) &&
    value.aggregate === undefined &&
    isPreviewUnavailableReason(value.unavailableReason)
  );
}
function isPreviewUnavailableReason(value: unknown): value is ResourceSchedulePreviewUnavailableReason {
  return ['missing-evidence', 'stale-evidence', 'mixed-currency', 'unsupported', 'calculation-failed'].includes(String(value));
}
export function isResourceSchedulingOpportunity(value: unknown): value is ResourceSchedulingOpportunity {
  ...
  return (
    value.projectionAvailability === 'unavailable' && value.projection === undefined && isPreviewUnavailableReason(value.projectionUnavailableReason)
  );
}
