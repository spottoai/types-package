import {
  ...
  type ResourceSchedulingCapabilityProjection,
  type ResourceSchedulingReadinessProjection,
  type ResourceSchedulingReadinessState,
...
export function isResourceSchedulingCapabilityProjection(value: unknown): value is ResourceSchedulingCapabilityProjection {
  ...
  if (
    !['allowed', 'blocked'].includes(String(executionPolicy.authoring)) ||
  ...
  if (
    !['none', 'capture-selected-state', 'capture-resource-snapshot'].includes(String(baseline.policy)) ||
  ...
  if (
    !['A-meter-stops', 'B-meter-reduces', 'C-no-material-saving'].includes(String(cost.billingClass)) ||
  ...
  if (
    !isPositiveInteger(cadence.minimumTransitionIntervalMinutes) ||
  ...
  if (
    !['reversible', 'conditional', 'destructive'].includes(String(disruption.reversibility)) ||
  ...
  if (
    !isNonNegativeInteger(restore.restoreLeadMinutes) ||
  ...
  if (
    minimumReducedWindowMinutes !== undefined &&
  ...
  const requiresDisableReason =
    value.lifecycleStatus === 'withdrawn' ||
...
export function isResourceSchedulingReadinessProjection(value: unknown): value is ResourceSchedulingReadinessProjection {
  return (
    ...
    readinessStates.has(value.state as ResourceSchedulingReadinessState) &&
