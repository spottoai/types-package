import { formatExactDecimalValue, parseCanonicalDecimal, sumCanonicalDecimals } from '../common/exactDecimal';
import type {
  FinancialAnalyticsDailyPointV1,
  FinancialAnalyticsDisplayBucketV1,
  FinancialAnalyticsDisplayPeriodKeyV1,
  FinancialAnalyticsDisplayProjectionV1,
  FinancialAnalyticsInputSeriesV1,
} from './financialAnalytics';
import { isFinancialAnalyticsInputSeriesV1 } from './financialAnalyticsValidation';
...
const shiftUtcMonths = (date: string, months: number): string => {
  const parsed = new Date(`${date}T00:00:00.000Z`);
  return new Date(Date.UTC(parsed.getUTCFullYear(), parsed.getUTCMonth() + months, 1)).toISOString().slice(0, 10);
};

const uniqueReasons = (values: Iterable<string>): [string, ...string[]] => {
  const reasons = [...new Set(values)].sort();
  return (reasons.length > 0 ? reasons : [HISTORY_NOT_PRODUCED]) as [string, ...string[]];
};

/** Twelve-decimal half-away-from-zero display average; totals remain exact. */
const averageKnownPerDay = (amount: string, dayCount: number): string => {
  if (!Number.isSafeInteger(dayCount) || dayCount < 1) throw new RangeError('Display average requires a positive day count.');
...
): FinancialAnalyticsInputSeriesV1['coordinate']['period']['requested'] => {
...
const displayDays = (
  ...
  const gapReasons = (date: string): string[] =>
    input.gaps
  ...
  for (let date = startDate; date < endDateExclusive; date = addUtcDays(date, 1)) {
    const point: FinancialAnalyticsDailyPointV1 | undefined = pointByDate.get(date);
...
const toBucket = (bucketKey: string, startDate: string, endDateExclusive: string, days: DisplayDay[]): FinancialAnalyticsDisplayBucketV1 => {
  const knownAmounts = days.flatMap(day => (day.knownAmount === undefined ? [] : [day.knownAmount]));
  const coversWholeBucket =
    days[0]?.date === startDate && days.length > 0 && addUtcDays(days[days.length - 1]!.date, 1) === endDateExclusive;
...
const dailyBuckets = (days: DisplayDay[]): FinancialAnalyticsDisplayBucketV1[] =>
  days.map(day => toBucket(day.date, day.date, addUtcDays(day.date, 1), [day]));

const monthlyBuckets = (days: DisplayDay[]): FinancialAnalyticsDisplayBucketV1[] => {
  const byMonth = new Map<string, DisplayDay[]>();
  days.forEach(day => {
    const month = day.date.slice(0, 7);
    byMonth.set(month, [...(byMonth.get(month) ?? []), day]);
  });
  return [...byMonth.entries()].map(([month, monthDays]) =>
    toBucket(month, `${month}-01`, shiftUtcMonths(`${month}-01`, 1), monthDays)
  );
};
...
export const projectFinancialAnalyticsDisplayPeriodV1 = (request: {
  ...
  const current = request.input.referenceCompositions.find(
    composition => composition.coordinate.periodRole === 'current-spend' && composition.compositionId === request.currentSpendCompositionId
  );
  if (!current) {
  ...
  const buckets =
    request.periodKey === 'trailing-12-calendar-months' ? monthlyBuckets(days) : dailyBuckets(days);
  const common = {
    ...
    requested,
    bucketGranularity: request.periodKey === 'trailing-12-calendar-months' ? ('calendar-month' as const) : ('daily' as const),
    currencyCode: accountingCurrency.currencyCode,
    buckets,
  };
  const knownAmounts = days.flatMap(day => (day.knownAmount === undefined ? [] : [day.knownAmount]));
  const calendarPeriodIncomplete =
    request.periodKey === 'trailing-12-calendar-months' &&
  ...
  if (knownAmounts.length === 0) return { ...common, status: 'unavailable', reasonCodes };
  const knownAmount = formatExactDecimalValue(sumCanonicalDecimals(knownAmounts));
  const average = averageKnownPerDay(knownAmount, knownAmounts.length);
