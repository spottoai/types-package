...
import {
  FINANCIAL_EVIDENCE_ASSESSMENT_CONTRACT_VERSION_V1,
  type FinancialEvidenceAssessmentIdentityPreimageV1,
  type FinancialEvidenceAssessmentV1,
} from './financialScopeEvidence';
import { canonicalizeValidatedFinancialEvidenceAssessmentIdentityV1 } from './financialScopeBaselineIdentity';
...
const isCountSummary = (value: unknown): value is { requestedRoleCount: number; producedRoleCount: number; matchedRoleCount: number } =>
  isRecord(value) &&
...
const isAssessmentIdentity = (value: unknown): value is FinancialEvidenceAssessmentIdentityPreimageV1 => {
  if (
    !isRecord(value) ||
    !hasExactFields(
      value,
  ...
  if (requestedRoles.size === 0 && !new Set(['subscription-aggregate', 'portfolio-currency-group']).has(value.request.scopeKind)) return false;
  ...
  if (new Set(value.roleAssessments.map(item => item.role)).size !== value.roleAssessments.length) return false;
  if (
    !new Set(['complete', 'partial', 'unavailable', 'not-applicable']).has(String(value.completeness)) ||
  ...
  return (
    value.summary.requestedRoleCount === requestedRoles.size &&
...
export const canonicalizeFinancialEvidenceAssessmentIdentityV1 = (value: FinancialEvidenceAssessmentIdentityPreimageV1): string => {
  if (!isAssessmentIdentity(value)) throw new TypeError('Invalid FinancialEvidenceAssessmentIdentityPreimageV1.');
  return canonicalizeValidatedFinancialEvidenceAssessmentIdentityV1(value);
};

export const isFinancialEvidenceAssessmentV1 = (value: unknown): value is FinancialEvidenceAssessmentV1 => {
  if (!isRecord(value) || !Object.prototype.hasOwnProperty.call(value, 'assessmentId')) return false;
