import type {
  FinancialEvidenceAssessmentIdentityPreimageV1,
  FinancialEvidenceBundleIdentityPreimageV1,
  FinancialEvidenceIntervalV1,
  FinancialEvidenceReferenceV1,
} from './financialScopeEvidence';
import type {
  FinancialBaselineCoverageV2,
  FinancialBaselinePeriodV2,
  FinancialOwnerBaselineComponentV2,
  FinancialScopeBaselineIdentityPreimageV2,
} from './financialScopeBaseline';

const compareCodePoints = (left: string, right: string): number => (left < right ? -1 : left > right ? 1 : 0);

const canonicalInterval = (interval: FinancialEvidenceIntervalV1): Record<string, unknown> => ({
...
const canonicalEvidenceReference = (reference: FinancialEvidenceReferenceV1): Record<string, unknown> => ({
...
export const canonicalizeValidatedFinancialEvidenceBundleIdentityV1 = (value: FinancialEvidenceBundleIdentityPreimageV1): string =>
  JSON.stringify({
...
export const canonicalizeValidatedFinancialEvidenceAssessmentIdentityV1 = (value: FinancialEvidenceAssessmentIdentityPreimageV1): string =>
  JSON.stringify({
    schemaVersion: value.schemaVersion,
    contractVersion: value.contractVersion,
...
const canonicalCoverage = (coverage: FinancialBaselineCoverageV2): Record<string, unknown> => ({
...
const canonicalPeriod = (period: FinancialBaselinePeriodV2): Record<string, unknown> => ({
...
const canonicalComponent = (component: FinancialOwnerBaselineComponentV2): Record<string, unknown> => ({
...
export const canonicalizeValidatedFinancialScopeBaselineIdentityV2 = (value: FinancialScopeBaselineIdentityPreimageV2): string => {
  const common = {
    schemaVersion: value.schemaVersion,
    contractVersion: value.contractVersion,
