...
export const isFinancialDataflowCoordinateV1 = (value: unknown): value is FinancialDataflowCoordinateV1 => {
  if (
    ...
    !hasFinancialDataflowExactFieldsV1(
      value,
      ['companyId', 'provider', 'providerAccountRefs', 'scope', 'periodRole', 'period', 'costBasis', 'estimateLens', 'accountingCurrency'],
...
export const isCurrentSpendCompositionCompatibleV1 = (composition: unknown, baselines: unknown): boolean => {
  if (
  ...
  for (const member of composition.members) {
    ...
    if (member.status === 'included') {
      if (baseline.status !== 'available' || member.baselineId !== baseline.baselineId) return false;
      if (
    ...
    if (baseline.status !== 'unavailable' || member.reasonCode !== baseline.unavailableReason) return false;
