export const FINANCIAL_PROJECTION_SCHEMA_VERSION_V1 = 1 as const;
...
export type FinancialProjectionOperationKindV1 =
  ...
  | 'schedule-quantity'
  | 'commitment-coverage';
export type FinancialProjectionTargetProvenanceV1 =
  | 'retail-derived'
  | 'provider-quote-derived'
  | 'billing-backed'
  | 'estimated'
...
export interface FinancialProjectionCommitmentCoverageV1 {
  ...
  eligibleQuantity: { amount: string; unit: string };
  existingCoveredQuantity: { amount: string; unit: string };
  coveredQuantity: { amount: string; unit: string };
  commitmentCharge: { amount: string; currencyCode: string };
  uncoveredQuantity: { amount: string; unit: string };
  uncoveredRate: { amount: string; currencyCode: string; quantityUnit: string };
...
export type FinancialProjectionAppliedComponentTargetV1 = FinancialProjectionAppliedComponentTargetCommonV1 &
  (
    | {
        sourceQuantity: { amount: string; unit: string };
    ...
    | {
        ...
        configurationTransformation: {
          ...
          targetQuantity: { amount: '0'; unit: string };
...
export const FINANCIAL_PROJECTION_UNAVAILABLE_REASONS_V1 = [
  'baseline-unavailable',
  'affected-component-not-found',
  'target-evidence-unavailable',
  'target-period-incompatible',
  'target-basis-incompatible',
  'target-currency-conflicting',
...
export type FinancialProjectionIdentityPreimageV1 = Omit<
  AvailableFinancialProjectionV1,
  'status' | 'projectionId' | 'current' | 'target' | 'change' | 'reconciliation'
>;
