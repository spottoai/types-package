import {
  FINANCIAL_AUTHORITY_RESOURCE_PROJECTION_CONTRACT_VERSION_V1,
  type FinancialAuthorityResourceProjectionV1,
  type FinancialAuthorityViewV1,
} from './financialAuthorityView';
import {
  FINANCIAL_SAVINGS_RESOURCE_PROJECTION_CONTRACT_VERSION_V1,
...
export const projectFinancialAuthorityResourceV1 = (
  authority: FinancialAuthorityViewV1,
  resourceType: string,
  scopeId: string
): FinancialAuthorityResourceProjectionV1 | undefined => {
  const normalizedType = normalize(resourceType);
  const normalizedScopeId = normalize(scopeId);
  const coverage = authority.scopeCoverage.find(
    entry => normalize(entry.resourceType) === normalizedType && entry.scopeIds.some(candidate => normalize(candidate) === normalizedScopeId)
  );
  ...
  const coordinates = authority.coordinates.map(coordinate => {
    ...
    if (owners.length !== 1 || !ownerBaseline) throw new TypeError('Financial authority resource owner is missing or ambiguous.');
    const chargeComposition =
      ownerBaseline.status === 'available' && ownerBaseline.baselineKind === 'owner'
  ...
  if (!firstCoordinate) throw new TypeError('Financial authority resource projection has no coordinates.');
  ...
  coordinates.forEach(coordinate => {
    ...
    coordinate.projections.forEach(projection => {
      ...
      if (projection.targetEvidenceBundleId) bundleIds.add(projection.targetEvidenceBundleId);
    });
  });
  ...
  evidenceAssessments.forEach(assessment => {
    if (assessment.evidenceBundleId) bundleIds.add(assessment.evidenceBundleId);
  });
...
export const projectFinancialSavingsResourceV1 = (
  ...
  if (
    financialCoordinateIds.size !== financialProjection.coordinates.length ||
  ...
  const coordinates = financialProjection.coordinates.map(financialCoordinate => {
    ...
    return unavailableScenarioIds.length > 0
      ...
      : { ...composed, status: 'available' as const };
  });
  const firstCoordinate = coordinates[0];
  if (!firstCoordinate) throw new TypeError('Financial savings resource projection has no coordinates.');
