...
import {
  FINANCIAL_PROJECTION_CONTRACT_VERSION_V1,
  FINANCIAL_PROJECTION_SCHEMA_VERSION_V1,
  FINANCIAL_PROJECTION_UNAVAILABLE_REASONS_V1,
  type FinancialProjectionEnvelopeV1,
  type FinancialProjectionAppliedComponentTargetV1,
  type FinancialProjectionIdentityPreimageV1,
  type FinancialProjectionNormalizedAverageMonthV1,
  type FinancialProjectionObservedPeriodProfileV1,
...
const UNAVAILABLE: ReadonlySet<string> = new Set(FINANCIAL_PROJECTION_UNAVAILABLE_REASONS_V1);
...
const isHashArray = (value: unknown): value is [string, ...string[]] =>
  Array.isArray(value) && value.length > 0 && value.length <= 20_000 && value.every(isHash) && new Set(value).size === value.length;
const isPossiblyEmptyHashArray = (value: unknown): value is string[] =>
  Array.isArray(value) && value.length <= 20_000 && value.every(isHash) && new Set(value).size === value.length;
const isDecimal = (value: unknown, currency = 'AUD'): value is string =>
  typeof value === 'string' && isCanonicalExactMoney({ amount: value, currencyCode: currency });
const isNonNegativeDecimal = (value: unknown, currency = 'AUD'): value is string => {
  if (!isDecimal(value, currency)) return false;
  try {
...
const isObservedPeriodProfile = (value: unknown): value is FinancialProjectionObservedPeriodProfileV1 =>
  isRecord(value) &&
...
const isTargetPeriodProfileCompatible = (convention: unknown, profile: unknown, available: boolean): boolean =>
  convention === 'normalized-average-month'
    ...
    : convention === 'same-period-quantity'
      ...
      : profile === undefined;
const equalsDecimal = (left: ReturnType<typeof parseCanonicalDecimal>, right: ReturnType<typeof parseCanonicalDecimal>): boolean =>
  formatExactDecimalValue(left) === formatExactDecimalValue(right);
const roundRationalHalfEven = (numerator: ReturnType<typeof parseCanonicalDecimal>, denominator: bigint, targetScale: number): string => {
...
const isCommitmentCoverage = (value: unknown, targetAmount: string, currency: string, profile: unknown): boolean => {
  if (
    !isRecord(value) ||
  ...
  if (
    quantities.some(quantity => !hasExactFields(quantity, ['amount', 'unit']) || !isNonNegativeDecimal(quantity.amount) || !isUnit(quantity.unit)) ||
  ...
  if (!equalsDecimal(eligible, allocated)) return false;
  const uncoveredCharge = multiplyExactDecimalValues(
    parseCanonicalDecimal(String(value.uncoveredQuantity.amount)),
    parseCanonicalDecimal(String(value.uncoveredRate.amount))
  );
  const replayedTarget = addExactDecimalValues(parseCanonicalDecimal(String(value.commitmentCharge.amount)), uncoveredCharge);
  if (!equalsDecimal(replayedTarget, parseCanonicalDecimal(targetAmount))) return false;
  if (
    !hasExactFields(value.quote, ['kind', 'amount', 'currencyCode', 'termMonths', 'termDayCount']) ||
  ...
  if (purchaseQuantity.scale !== 0) return false;
  ...
  if (
    !Number.isSafeInteger(value.quote.termDayCount) ||
  ...
  if (
    !capacityHours ||
    subtractExactDecimalValues(
      parseCanonicalDecimal(String(value.coveredQuantity.amount)),
      multiplyExactDecimalValues(purchaseQuantity, parseCanonicalDecimal(capacityHours))
    ).coefficient > 0n
  )
    return false;
  if (isNormalizedAverageMonthProfile(profile)) {
    return equalsDecimal(
      multiplyExactDecimalValues(quoteAmount, purchaseQuantity),
      multiplyExactDecimalValues(commitmentCharge, parseCanonicalDecimal(String(value.quote.termMonths)))
    );
  }
  if (!isObservedPeriodProfile(profile)) return false;
  const allocatedQuote = multiplyExactDecimalValues(
    multiplyExactDecimalValues(quoteAmount, purchaseQuantity),
    parseCanonicalDecimal(String(profile.dayCount))
  );
  return (
    roundRationalHalfEven(allocatedQuote, BigInt(Number(value.quote.termDayCount)), Number(profile.currencyMinorUnitScale)) ===
    formatExactDecimalValue(commitmentCharge)
  );
};
const isAppliedComponentTargets = (
  value: unknown,
  operationKind: unknown,
  targetProvenance: unknown,
  targetPeriodProfile: unknown,
  accountingCurrencyCode: unknown,
  affectedComponentIds: unknown
): value is [FinancialProjectionAppliedComponentTargetV1, ...FinancialProjectionAppliedComponentTargetV1[]] => {
  if (
  ...
  for (const target of value) {
    ...
    if (operationKind === 'replace-rate') {
      if (
        !hasExactFields(target, ['componentId', 'targetAmount', 'targetConfigurationId', 'targetEvidenceRefIds', 'sourceQuantity', 'targetRate']) ||
    ...
    if (operationKind === 'replace-quantity' || operationKind === 'schedule-quantity') {
      if (
        !hasExactFields(target, ['componentId', 'targetAmount', 'targetConfigurationId', 'targetEvidenceRefIds', 'sourceRate', 'targetQuantity']) ||
...
const isSameHashSet = (left: readonly string[], right: readonly string[]): boolean =>
...
const isProjectionIdentity = (value: unknown): value is FinancialProjectionIdentityPreimageV1 =>
  isRecord(value) &&
  hasExactFields(
    value,
    [
  ...
  isAppliedComponentTargets(
    value.appliedComponentTargets,
    value.operationKind,
    value.targetProvenance,
    value.targetPeriodProfile,
    value.accountingCurrencyCode,
    value.affectedComponentIds
  ) &&
...
export const canonicalizeFinancialProjectionIdentityV1 = (value: FinancialProjectionIdentityPreimageV1): string => {
  if (!isProjectionIdentity(value)) throw new TypeError('Invalid FinancialProjectionIdentityPreimageV1.');
...
const isAmounts = (value: unknown, currency: string): value is { total: string; affected: string; unchanged: string } =>
  isRecord(value) &&
...
const isAvailable = (value: JsonRecord): boolean => {
  ...
  if (
    ...
    return false;
  const appliedTargets = value.appliedComponentTargets as FinancialProjectionAppliedComponentTargetV1[];
  if (
    formatExactDecimalValue(sumCanonicalDecimals(appliedTargets.map(target => target.targetAmount))) !==
    (value.target as { affected: string }).affected
  )
    return false;
  const current = value.current as { total: string };
  const target = value.target as { total: string };
  const change = value.change as { delta: string; savings: string; increase: string };
  ...
  if (change.delta !== expectedDelta || change.savings !== expectedSavings || change.increase !== expectedIncrease) return false;
  const {
    status: _status,
...
export const isFinancialProjectionEnvelopeV1 = (value: unknown): value is FinancialProjectionEnvelopeV1 =>
  isRecord(value) && (isAvailable(value) || isUnavailable(value));
