import { formatExactDecimalValue, parseCanonicalDecimal, subtractExactDecimalValues } from '../common/exactDecimal';
import { sha256Utf8 } from '../common/sha256';
import type {
  FinancialResourceScenarioV1,
  FinancialResourceSurfaceProjectionIdentityPreimageV1,
  FinancialResourceSurfaceProjectionV1,
  FinancialResourceSurfaceValueV1,
} from './financialResourceSurfaceProjection';
import {
  FINANCIAL_RESOURCE_SURFACE_PROJECTION_CONTRACT_VERSION_V1,
  FINANCIAL_RESOURCE_SURFACE_PROJECTION_SCHEMA_VERSION_V1,
} from './financialResourceSurfaceProjection';
import {
  isFinancialSurfaceCoordinateDefinitionV1,
  isFinancialSurfaceMoneyV1,
} from './financialCurrentSpendSurfaceProjectionValidation';
import {
...
const MAX_SCENARIOS_PER_RESOURCE = 1_024;
...
const isExactAmount = (amount: unknown, currencyCode: unknown): amount is string =>
  isCurrency(currencyCode) && isCanonicalExactMoney({ amount, currencyCode });

const isScenario = (value: unknown, coordinateIds: ReadonlySet<string>, requestedCurrencyCode?: string): value is FinancialResourceScenarioV1 => {
  ...
  if (value.status === 'available') {
    ...
    return (
      ...
      formatExactDecimalValue(
        subtractExactDecimalValues(parseCanonicalDecimal(value.targetAmount), parseCanonicalDecimal(value.currentAmount))
      )
    );
  }
  if (value.status === 'unavailable') {
    return hasExactFinancialSavingsFields(
      value,
      ['scenarioId', 'coordinateId', 'category', 'additivity', 'status', 'reasonCodes'],
      optionalCommon
  ...
  if (
    !hasExactFinancialSavingsFields(
      value,
      ['scenarioId', 'coordinateId', 'category', 'additivity', 'status', 'reasonCodes'],
      [...optionalCommon, 'currentAmount', 'targetAmount', 'changeAmount', 'currencyCode']
    ) ||
    !isReasonCodes(value.reasonCodes)
  )
    return false;
  ...
  if (presentAmounts.length === 0) return value.currencyCode === undefined;
  return (
    isCurrency(value.currencyCode) &&
    (requestedCurrencyCode === undefined || value.currencyCode === requestedCurrencyCode) &&
    presentAmounts.every(amount => isExactAmount(amount, value.currencyCode))
  );
};

const canonicalizeValue = (value: FinancialResourceSurfaceValueV1): FinancialResourceSurfaceValueV1 =>
  value.status === 'available' ? value : { ...value, reasonCodes: [...value.reasonCodes].sort() as [string, ...string[]] };

const canonicalizeScenario = (value: FinancialResourceScenarioV1): FinancialResourceScenarioV1 =>
  value.status === 'available' ? value : { ...value, reasonCodes: [...value.reasonCodes].sort() as [string, ...string[]] };

export const canonicalizeFinancialResourceSurfaceProjectionIdentityV1 = (
  value: FinancialResourceSurfaceProjectionIdentityPreimageV1
): string =>
  JSON.stringify(
    canonicalizeFinancialSavingsJsonValue({
      ...value,
          ...
          ...(resource.scenarios === undefined
            ...
            : {
                scenarios: [...resource.scenarios]
                  .sort(
                    (left, right) =>
                      left.scenarioId.localeCompare(right.scenarioId) || left.coordinateId.localeCompare(right.coordinateId)
                  )
...
export const createFinancialResourceSurfaceProjectionIdV1 = (
  value: FinancialResourceSurfaceProjectionIdentityPreimageV1
): string => `sha256:${sha256Utf8(canonicalizeFinancialResourceSurfaceProjectionIdentityV1(value))}`;
 ...
 */
export const projectFinancialResourceSurfaceMemberV1 = (
  source: FinancialResourceSurfaceProjectionV1,
  resourceId: string
): FinancialResourceSurfaceProjectionV1 | undefined => {
  ...
  const member = source.resources.find(
    candidate => candidate.resourceId.trim().toLowerCase().replace(/\/+$/, '') === normalizedResourceId
  );
  ...
  const identity: FinancialResourceSurfaceProjectionIdentityPreimageV1 = {
    schemaVersion: source.schemaVersion,
    contractVersion: source.contractVersion,
    provider: source.provider,
    providerAccountRefs: [...source.providerAccountRefs] as [string, ...string[]],
    artifactGeneration: source.artifactGeneration,
    financialAuthorityId: source.financialAuthorityId,
    coordinates: source.coordinates.filter(coordinate => coordinateIds.has(coordinate.coordinateId)) as [
      typeof source.coordinates[number],
      ...typeof source.coordinates[number][],
    ],
    resources: [member],
  };
  const projection: FinancialResourceSurfaceProjectionV1 = {
    ...identity,
 ...
 * financial artifact.
 */
export const projectFinancialResourceSurfaceWithoutScenariosV1 = (
  source: FinancialResourceSurfaceProjectionV1
): FinancialResourceSurfaceProjectionV1 => {
  if (!isFinancialResourceSurfaceProjectionV1(source)) {
    throw new TypeError('Invalid FinancialResourceSurfaceProjectionV1.');
  ...
  const identity: FinancialResourceSurfaceProjectionIdentityPreimageV1 = {
    ...
    artifactGeneration: source.artifactGeneration,
    financialAuthorityId: source.financialAuthorityId,
    coordinates: source.coordinates.filter(coordinate => referencedCoordinateIds.has(coordinate.coordinateId)) as [
      typeof source.coordinates[number],
      ...typeof source.coordinates[number][],
    ],
...
export const isFinancialResourceSurfaceProjectionV1 = (value: unknown): value is FinancialResourceSurfaceProjectionV1 => {
  if (
  ...
  for (const resource of value.resources) {
    if (
      !isFinancialSavingsRecord(resource) ||
    ...
    const valueCoordinates = new Set<string>();
    for (const memberValue of resource.values) {
      if (!isFinancialSavingsRecord(memberValue) || !isFinancialSavingsHash(memberValue.coordinateId)) return false;
      const definition = coordinateById.get(memberValue.coordinateId);
      if (definition === undefined || valueCoordinates.has(memberValue.coordinateId)) return false;
      const money = Object.fromEntries(Object.entries(memberValue).filter(([key]) => key !== 'coordinateId'));
      if (!isFinancialSurfaceMoneyV1(money, definition.requestedCurrencyCode)) return false;
      valueCoordinates.add(memberValue.coordinateId);
    }
    if (resource.scenarios !== undefined) {
      if (!Array.isArray(resource.scenarios) || resource.scenarios.length > MAX_SCENARIOS_PER_RESOURCE) return false;
      const scenarioCoordinates = new Set<string>();
      for (const scenario of resource.scenarios) {
        if (!isFinancialSavingsRecord(scenario) || !isFinancialSavingsHash(scenario.coordinateId)) return false;
  ...
  const projection = value as unknown as FinancialResourceSurfaceProjectionV1;
