...
import {
  FINANCIAL_AUTHORITY_VIEW_CONTRACT_VERSION_V1,
  FINANCIAL_AUTHORITY_VIEW_SCHEMA_VERSION_V1,
  type FinancialAuthorityCoordinateIdentityPreimageV1,
  type FinancialAuthorityCoordinateV1,
  type FinancialAuthorityResourceCoordinateV1,
  type FinancialAuthorityResourceProjectionV1,
  type FinancialAuthorityScopeCoverageV1,
  type FinancialAuthorityViewIdentityPreimageV1,
  type FinancialAuthorityViewV1,
} from './financialAuthorityView';
import { isFinancialProjectionEnvelopeV1 } from './financialProjectionValidation';
import { isFinancialEvidenceAssessmentV1 } from './financialEvidenceAssessmentValidation';
import type { FinancialEvidenceAssessmentV1 } from './financialScopeEvidence';
import { formatExactDecimalValue, multiplyExactDecimalValues, parseCanonicalDecimal, sumCanonicalDecimals } from '../common/exactDecimal';
...
const normalizeScopeId = (value: string): string => value.trim().toLowerCase().replace(/\/+$/, '');
...
const canonicalText = (value: unknown): string => JSON.stringify(canonicalJsonValue(value));
const periodText = (period: unknown): string => canonicalText(period);
const baselineAmount = (baseline: FinancialScopeBaselineEnvelopeV2): string | undefined =>
  baseline.status === 'available' ? baseline.total.amount : undefined;
const exactSumEquals = (left: string, right: string, expected: string): boolean => {
  try {
...
const canonicalCoordinatePreimage = (value: FinancialAuthorityCoordinateIdentityPreimageV1): unknown => ({
...
export const canonicalizeFinancialAuthorityCoordinateIdentityV1 = (value: FinancialAuthorityCoordinateIdentityPreimageV1): string =>
  canonicalText(canonicalCoordinatePreimage(value));

export const createFinancialAuthorityCoordinateIdV1 = (value: FinancialAuthorityCoordinateIdentityPreimageV1): string =>
  `sha256:${sha256Utf8(canonicalizeFinancialAuthorityCoordinateIdentityV1(value))}`;

const canonicalAuthorityPreimage = (value: FinancialAuthorityViewIdentityPreimageV1): unknown => ({
...
export const canonicalizeFinancialAuthorityViewIdentityV1 = (value: FinancialAuthorityViewIdentityPreimageV1): string =>
  canonicalText(canonicalAuthorityPreimage(value));

export const createFinancialAuthorityViewIdV1 = (value: FinancialAuthorityViewIdentityPreimageV1): string =>
  `sha256:${sha256Utf8(canonicalizeFinancialAuthorityViewIdentityV1(value))}`;

const isSameStringSet = (left: string[], right: string[]): boolean => {
...
const isCoordinate = (
  ...
  financialRoleByScope: Map<string, FinancialAuthorityScopeCoverageV1['financialRole']>,
  providerAccountRefs: string[],
  assessmentById: Map<string, FinancialEvidenceAssessmentV1>,
  bundleIds: Set<string>,
  bundleById: Map<string, FinancialAuthorityViewV1['evidenceBundles'][number]>
): value is FinancialAuthorityCoordinateV1 => {
  if (
    !isRecord(value) ||
    !hasExactFields(
      value,
    ...
    value.ownerBaselines.length > 20_000 ||
    ...
    value.displayRollups.length > 20_000 ||
    !value.displayRollups.every(isFinancialDisplayRollupV1) ||
    !Array.isArray(value.projections) ||
    value.projections.length > 20_000 ||
    !value.projections.every(isFinancialProjectionEnvelopeV1)
  )
    return false;

  const coordinate = value as unknown as FinancialAuthorityCoordinateV1;
  const ownerScopeIds = coordinate.ownerBaselines.map(baseline => baseline.scopeId);
  if (!isSameStringSet(ownerScopeIds, coveredScopeIds) || new Set(ownerScopeIds).size !== ownerScopeIds.length) return false;
  ...
  for (const baseline of allBaselines) {
    ...
    if (baseline.status === 'available' && baseline.baselineKind === 'owner') {
      ...
      if (!evidenceBundle) return false;
      ...
      if (
        currencyReferences.some(reference => reference === undefined) ||
        !currencyReferences.some(
          reference => reference?.role === 'billing' || reference?.role === 'estimate' || reference?.role === 'billing-currency-declaration'
        )
      )
        return false;
      if (
        baseline.accountingCurrency.fxEvidenceRefId !== undefined &&
  ...
  for (const baseline of coordinate.ownerBaselines) {
    if (baseline.scopeKind !== 'canonical-resource-owner' && baseline.scopeKind !== 'composite-resource') return false;
    const financialRole = financialRoleByScope.get(baseline.scopeId);
    if (
      financialRole === undefined ||
  ...
  if (coordinate.aggregateBaseline.status === 'available') {
    if (!everyMemberAvailable || !isSameStringSet([...coordinate.aggregateBaseline.memberBaselineIds], availableMembers)) return false;
  } else if (everyMemberAvailable) {
    return false;
  }

  const ownerByScope = new Map(coordinate.ownerBaselines.map(baseline => [baseline.scopeId, baseline]));
  const componentKey = (baselineId: string, componentId: string): string => `${baselineId}\u0000${componentId}`;
  const componentByKey = new Map<
    string,
    {
      baseline: Extract<FinancialScopeBaselineEnvelopeV2, { status: 'available'; baselineKind: 'owner' }>;
      component: Extract<FinancialScopeBaselineEnvelopeV2, { status: 'available'; baselineKind: 'owner' }>['components'][number];
    }
  >();
  for (const baseline of coordinate.ownerBaselines) {
    if (baseline.status !== 'available' || baseline.baselineKind !== 'owner') continue;
    for (const component of baseline.components) {
      const key = componentKey(baseline.baselineId, component.componentId);
      if (componentByKey.has(key)) return false;
      componentByKey.set(key, { baseline, component });
    }
  }
  const descriptorByKey = new Map<string, FinancialAuthorityComponentDescriptorV1>();
  ...
  if (descriptorByKey.size !== componentByKey.size) return false;
  ...
  for (const rollup of coordinate.displayRollups) {
    if (rollupIds.has(rollup.displayRollupId)) return false;
    ...
    for (const member of rollup.members) {
      ...
      if (!owned || !descriptor) return false;
      const scopedKey = `${normalizeScopeId(rollup.displayScopeId)}\u0000${rollup.purpose}\u0000${key}`;
      if (scopedMemberships.has(scopedKey)) return false;
      scopedMemberships.add(scopedKey);
      if (
        (rollup.displayLabelSource === 'service-name' && descriptor.serviceName !== rollup.displayLabel) ||
  ...
  if ([...componentByKey.keys()].some(key => primaryMembershipCounts.get(key) !== 1)) return false;

  const scenarios = new Set<string>();
  for (const projection of coordinate.projections) {
    const scenarioKey = `${projection.scopeId}|${projection.scenarioId}`;
    if (scenarios.has(scenarioKey) || !ownerByScope.has(projection.scopeId) || financialRoleByScope.get(projection.scopeId) !== 'owner') return false;
    ...
    if (
      projection.accountingCurrencyCode !== coordinate.requestedCurrencyCode ||
    ...
    if (projection.status === 'available' && owner.status === 'available' && owner.baselineKind === 'owner') {
      ...
      if (projection.targetPeriodConvention === 'normalized-average-month') return false;
      ...
      try {
        const affectedAmount = formatExactDecimalValue(
          sumCanonicalDecimals(
            owner.components.filter(component => affectedComponentIds.has(component.componentId)).map(component => component.amount)
          )
        );
        const unchangedAmount = formatExactDecimalValue(
          sumCanonicalDecimals(
            owner.components.filter(component => !affectedComponentIds.has(component.componentId)).map(component => component.amount)
          )
        );
        if (
          projection.current.total !== owner.total.amount ||
        ...
        for (const applied of projection.appliedComponentTargets) {
          ...
          if (!component || applied.targetEvidenceRefIds.some(evidenceRefId => !targetEvidenceRefIds.has(evidenceRefId))) return false;
          if (projection.operationKind === 'commitment-coverage') {
            if (!('commitmentCoverage' in applied) || component.quantity === undefined || component.effectiveRate === undefined) return false;
            const commitment = applied.commitmentCoverage;
            if (
              commitment.eligibleQuantity.amount !== component.quantity.amount ||
              ...
              !applied.targetEvidenceRefIds.some(
                evidenceRefId => targetBundle.references.find(reference => reference.evidenceRefId === evidenceRefId)?.role === 'commitment-quote'
              )
            )
              return false;
            continue;
          ...
          if (projection.operationKind === 'remove-component') {
            if (applied.targetAmount !== '0') return false;
            continue;
          }
          if (projection.operationKind === 'replace-rate') {
            if (!('sourceQuantity' in applied) || !('targetRate' in applied) || component.quantity === undefined) return false;
            if (
              applied.sourceQuantity.amount !== component.quantity.amount ||
          ...
          if (!('sourceRate' in applied) || !('targetQuantity' in applied) || component.effectiveRate === undefined) return false;
          if (
            applied.sourceRate.amount !== component.effectiveRate.amount ||
            ...
            formatExactDecimalValue(
              multiplyExactDecimalValues(parseCanonicalDecimal(applied.sourceRate.amount), parseCanonicalDecimal(applied.targetQuantity.amount))
            ) !== applied.targetAmount
          )
            return false;
        }
...
const hasReconciledEstimateLensGroups = (coordinates: FinancialAuthorityCoordinateV1[]): boolean => {
  const groups = new Map<string, Map<string, FinancialAuthorityCoordinateV1>>();
  for (const coordinate of coordinates) {
    ...
    const group = groups.get(key) ?? new Map<string, FinancialAuthorityCoordinateV1>();
    if (group.has(coordinate.estimateLens)) return false;
  ...
  for (const group of groups.values()) {
    ...
    if (!actual || !combined || !estimated || group.size !== 3) return false;
    const baselineSets: ReadonlyArray<
      readonly [FinancialScopeBaselineEnvelopeV2, FinancialScopeBaselineEnvelopeV2, FinancialScopeBaselineEnvelopeV2]
    > = [
      ...combined.ownerBaselines.map((baseline, index) => [actual.ownerBaselines[index]!, baseline, estimated.ownerBaselines[index]!] as const),
    ...
    for (const [actualBaseline, combinedBaseline, estimatedBaseline] of baselineSets) {
      ...
      if (actualAmount !== undefined && combinedAmount !== undefined && estimatedAmount !== undefined) {
        if (!exactSumEquals(actualAmount, estimatedAmount, combinedAmount)) return false;
      }
      if (
        actualBaseline.status === 'available' &&
        ...
        for (const identity of identities) {
          if (
            !exactSumEquals(actualByIdentity.get(identity) ?? '0', estimatedByIdentity.get(identity) ?? '0', combinedByIdentity.get(identity) ?? '0')
          ) {
            return false;
          }
        }
...
export const isFinancialAuthorityViewV1 = (value: unknown): value is FinancialAuthorityViewV1 => {
  if (
    !isRecord(value) ||
    ...
    value.scopeCoverage.length > 1_024 ||
    !Array.isArray(value.evidenceBundles) ||
    value.evidenceBundles.length > 20_000 ||
    ...
    value.evidenceAssessments.length > 20_000 ||
  ...
  const authority = value as unknown as FinancialAuthorityViewV1;
  const coveredScopeIds: string[] = [];
  const financialRoleByScope = new Map<string, FinancialAuthorityScopeCoverageV1['financialRole']>();
  const coverageGroups = new Set<string>();
  for (const coverage of authority.scopeCoverage) {
    if (
      ...
      coverage.scopeIds.length > 20_000 ||
  ...
  if (new Set(coveredScopeIds).size !== coveredScopeIds.length) return false;
  ...
  if (bundleIds.size !== authority.evidenceBundles.length || assessmentById.size !== authority.evidenceAssessments.length) return false;
  const evidenceById = new Map<string, string>();
  for (const reference of authority.evidenceBundles.flatMap(bundle => bundle.references)) {
    if (Date.parse(reference.intrinsicTime.at) > Date.parse(authority.artifactGeneration.generatedAt)) return false;
    const referenceText = canonicalText(reference);
    const existing = evidenceById.get(reference.evidenceRefId);
    if (existing !== undefined && existing !== referenceText) return false;
    evidenceById.set(reference.evidenceRefId, referenceText);
  }
  if (
    authority.evidenceAssessments.some(assessment => assessment.evidenceBundleId !== undefined && !bundleIds.has(assessment.evidenceBundleId)) ||
    !authority.coordinates.every(coordinate =>
      isCoordinate(coordinate, coveredScopeIds, financialRoleByScope, [...authority.providerAccountRefs], assessmentById, bundleIds, bundleById)
    ) ||
...
export const isFinancialAuthorityViewBoundToArtifactGenerationV1 = (
  value: unknown,
  expected: ArtifactGeneration
): value is FinancialAuthorityViewV1 =>
  isFinancialAuthorityViewV1(value) &&
  value.artifactGeneration.runId === expected.runId &&
  value.artifactGeneration.generatedAt === expected.generatedAt;

const isSameProviderAccountSet = (left: readonly string[], right: readonly string[]): boolean => {
  if (left.length !== right.length) return false;
  const sortedLeft = [...left].sort();
  const sortedRight = [...right].sort();
...
const isResourceProjectionCoordinate = (
  value: unknown,
  projection: FinancialAuthorityResourceProjectionV1,
  assessmentById: ReadonlyMap<string, FinancialEvidenceAssessmentV1>,
  bundleIds: ReadonlySet<string>
): value is FinancialAuthorityResourceCoordinateV1 => {
  if (
    !isRecord(value) ||
    !hasExactFields(
      value,
    ...
    (value.displayMemberBaselines !== undefined &&
      (!Array.isArray(value.displayMemberBaselines) ||
        value.displayMemberBaselines.length > 20_000 ||
        !value.displayMemberBaselines.every(isFinancialScopeBaselineEnvelopeV2))) ||
    ...
    (value.displayMemberChargeCompositions !== undefined &&
      (!Array.isArray(value.displayMemberChargeCompositions) ||
        value.displayMemberChargeCompositions.length > 20_000 ||
        !value.displayMemberChargeCompositions.every(isFinancialChargeCompositionV1))) ||
    !Array.isArray(value.componentDescriptors) ||
    value.componentDescriptors.length > 20_000 ||
    !value.componentDescriptors.every(isFinancialAuthorityComponentDescriptorV1) ||
    !Array.isArray(value.displayRollups) ||
  ...
  const coordinate = value as unknown as FinancialAuthorityResourceCoordinateV1;
  ...
  const assessment = assessmentById.get(baseline.assessmentId);
  if (
    normalizeScopeId(baseline.scopeId) !== projection.scopeId ||
    baseline.costBasis !== coordinate.costBasis ||
    baseline.estimateLens !== coordinate.estimateLens ||
    periodText(baseline.period) !== periodText(coordinate.period) ||
    baseline.requestedCurrencyCode !== coordinate.requestedCurrencyCode ||
  ...
  if (baseline.status !== 'available' || baseline.baselineKind !== 'owner') {
    return (
      displayMemberBaselines.length === 0 &&
  ...
  if (baseline.scopeKind !== 'canonical-resource-owner' && baseline.scopeKind !== 'composite-resource') return false;
  if (!bundleIds.has(baseline.evidenceBundleId) || assessment.evidenceBundleId !== baseline.evidenceBundleId) return false;
  const availableDisplayMemberBaselines = displayMemberBaselines.filter(
    (candidate): candidate is Extract<FinancialScopeBaselineEnvelopeV2, { status: 'available'; baselineKind: 'owner' }> =>
      candidate.status === 'available' && candidate.baselineKind === 'owner'
  );
  if (availableDisplayMemberBaselines.length !== displayMemberBaselines.length) return false;
  const projectedBaselines = [baseline, ...availableDisplayMemberBaselines];
  if (
    new Set(projectedBaselines.map(candidate => candidate.baselineId)).size !== projectedBaselines.length ||
  ...
  if (compositions.some(candidate => candidate === undefined)) return false;
  const compositionByBaselineId = new Map(compositions.map(candidate => [candidate!.baselineId, candidate!]));
  if (compositions.length !== projectedBaselines.length || compositionByBaselineId.size !== projectedBaselines.length) return false;
  ...
  const componentKeys = new Set(
    projectedBaselines.flatMap(projectedBaseline =>
      projectedBaseline.components.map(component => `${projectedBaseline.baselineId}\u0000${component.componentId}`)
    )
  );
  ...
  if (
    descriptorKeys.length !== componentKeys.size ||
    new Set(descriptorKeys).size !== descriptorKeys.length ||
    descriptorKeys.some(key => !componentKeys.has(key)) ||
    coordinate.displayRollups.some(
      rollup =>
        normalizeScopeId(rollup.displayScopeId) !== projection.scopeId ||
    ...
    return false;
  const rolledUpKeys = coordinate.displayRollups.flatMap(rollup =>
    rollup.members.map(member => `${member.baselineId}\u0000${member.componentId}`)
  );
  return (
    ownerComponentKeys.size > 0 &&
...
export const isFinancialAuthorityResourceProjectionV1 = (value: unknown): value is FinancialAuthorityResourceProjectionV1 => {
  if (
    !isRecord(value) ||
    !hasExactFields(value, [
      'contractVersion',
  ...
  const projection = value as unknown as FinancialAuthorityResourceProjectionV1;
  ...
  const projectedScopeIds = new Set([
    projection.scopeId,
    ...projection.coordinates.flatMap(coordinate =>
      (coordinate.displayMemberBaselines ?? []).map(baseline => normalizeScopeId(baseline.scopeId))
    ),
  ]);
  if (bundleIds.size !== projection.evidenceBundles.length || assessmentById.size !== projection.evidenceAssessments.length) return false;
  if (
    projection.evidenceAssessments.some(
      assessment =>
        (assessment.evidenceBundleId !== undefined && !bundleIds.has(assessment.evidenceBundleId)) ||
