...
import {
  FINANCIAL_EVIDENCE_BUNDLE_CONTRACT_VERSION_V1,
  type FinancialEvidenceBundleIdentityPreimageV1,
  type FinancialEvidenceBundleV1,
  type FinancialEvidenceIntervalV1,
  type FinancialEvidenceReferenceV1,
} from './financialScopeEvidence';
import {
  FINANCIAL_SCOPE_BASELINE_CONTRACT_VERSION_V2,
  FINANCIAL_SCOPE_BASELINE_SCHEMA_VERSION_V2,
  FINANCIAL_SCOPE_BASELINE_UNAVAILABLE_REASONS_V2,
  type FinancialBaselineCoverageV2,
  type FinancialBaselinePeriodV2,
  type FinancialOwnerBaselineComponentV2,
  type FinancialScopeBaselineEnvelopeV2,
  type FinancialScopeBaselineIdentityPreimageV2,
} from './financialScopeBaseline';
import {
  canonicalizeValidatedFinancialEvidenceBundleIdentityV1,
  canonicalizeValidatedFinancialScopeBaselineIdentityV2,
} from './financialScopeBaselineIdentity';
...
const UNAVAILABLE_REASONS: ReadonlySet<string> = new Set(FINANCIAL_SCOPE_BASELINE_UNAVAILABLE_REASONS_V2);
...
const isInterval = (value: unknown): value is FinancialEvidenceIntervalV1 =>
  isRecord(value) &&
...
const isEvidenceReference = (value: unknown): value is FinancialEvidenceReferenceV1 => {
  if (
    !isRecord(value) ||
    !hasExactFields(
      value,
      ['evidenceRefId', 'role', 'sourceKind', 'digestAlgorithm', 'evidenceDigest', 'intrinsicTime'],
      ['generationId', 'revisionId', 'effectivePeriod']
    ) ||
...
export const isFinancialEvidenceBundleIdentityPreimageV1 = (value: unknown): value is FinancialEvidenceBundleIdentityPreimageV1 =>
  isRecord(value) &&
  hasExactFields(value, ['schemaVersion', 'contractVersion', 'references']) &&
  value.schemaVersion === 1 &&
  value.contractVersion === FINANCIAL_EVIDENCE_BUNDLE_CONTRACT_VERSION_V1 &&
...
export const canonicalizeFinancialEvidenceBundleIdentityV1 = (value: FinancialEvidenceBundleIdentityPreimageV1): string => {
  if (!isFinancialEvidenceBundleIdentityPreimageV1(value)) {
    throw new TypeError('Invalid FinancialEvidenceBundleIdentityPreimageV1.');
  }
  return canonicalizeValidatedFinancialEvidenceBundleIdentityV1(value);
};

export const isFinancialEvidenceBundleV1 = (value: unknown): value is FinancialEvidenceBundleV1 => {
  if (!isRecord(value) || !hasExactFields(value, ['schemaVersion', 'contractVersion', 'bundleId', 'references'])) return false;
  const { bundleId, ...identity } = value;
  return (
    typeof bundleId === 'string' &&
    SHA256_ID.test(bundleId) &&
    isFinancialEvidenceBundleIdentityPreimageV1(identity) &&
    bundleId === `sha256:${sha256Utf8(canonicalizeValidatedFinancialEvidenceBundleIdentityV1(identity))}`
  );
};

const isHashArray = (value: unknown, allowEmpty = false): value is [string, ...string[]] =>
  Array.isArray(value) &&
...
const isCoverage = (value: unknown): value is FinancialBaselineCoverageV2 =>
  isRecord(value) &&
...
export const isFinancialBaselinePeriodV2 = (value: unknown): value is FinancialBaselinePeriodV2 => {
  if (
    !isRecord(value) ||
    ...
    value.coverage.length > 20_000 ||
    !value.coverage.every(isCoverage) ||
    new Set(value.coverage.map(item => item.coverageId)).size !== value.coverage.length ||
    !Array.isArray(value.gaps) ||
    value.gaps.length > 20_000 ||
    !value.gaps.every(isInterval)
  ) {
    return false;
  }
  const requested = value.requested as FinancialEvidenceIntervalV1;
  const sameCalendar = (interval: FinancialEvidenceIntervalV1, reference: FinancialEvidenceIntervalV1): boolean =>
    interval.dateBasis === reference.dateBasis && interval.timeZone === reference.timeZone;
  const requestedDays = (Date.parse(`${requested.endDateExclusive}T00:00:00.000Z`) - Date.parse(`${requested.startDate}T00:00:00.000Z`)) / 86_400_000;
  if (value.windowKind === 'daily' && requestedDays !== 1) return false;
  if (value.windowKind === 'rolling-30-days' && requestedDays !== 30) return false;
  if (value.windowKind === 'calendar-month') {
    ...
    if (!requested.startDate.endsWith('-01') || requested.endDateExclusive !== nextMonthStart) return false;
  }
  if ((value.windowKind === 'provider-billing-period') !== (value.providerBillingPeriodId !== undefined)) return false;
  if (
    !value.gaps.every(
      gap => gap.startDate >= requested.startDate && gap.endDateExclusive <= requested.endDateExclusive && sameCalendar(gap, requested)
    )
  ) {
    return false;
  }
  if (value.observed === undefined) return value.coverage.length === 0;
  const observed = value.observed as FinancialEvidenceIntervalV1;
  if (
    ...
    !value.coverage.every(
      item =>
        item.interval.startDate >= observed.startDate &&
  ...
  const coversInterval = (startDate: string, endDateExclusive: string, intervals: readonly FinancialEvidenceIntervalV1[]): boolean => {
    ...
    for (const interval of sorted) {
      if (interval.startDate > cursor) return false;
      if (interval.endDateExclusive > cursor) cursor = interval.endDateExclusive;
    }
    return cursor === endDateExclusive;
  };
  const coverageIntervals = value.coverage.map(item => item.interval);
  return (
    coversInterval(observed.startDate, observed.endDateExclusive, coverageIntervals) ||
    coversInterval(requested.startDate, requested.endDateExclusive, [...coverageIntervals, ...value.gaps])
  );
};

const isCommonBaseline = (value: JsonRecord): boolean =>
  value.schemaVersion === FINANCIAL_SCOPE_BASELINE_SCHEMA_VERSION_V2 &&
  value.contractVersion === FINANCIAL_SCOPE_BASELINE_CONTRACT_VERSION_V2 &&
...
const isComponent = (
  ...
  estimateLens: unknown
): value is FinancialOwnerBaselineComponentV2 => {
  if (
    !isRecord(value) ||
    !hasExactFields(
      value,
      ['componentId', 'billableIdentity', 'ownerScopeId', 'chargeClassification', 'amount', 'evidenceRefIds', 'coverageIds'],
      ['estimateReason', 'quantity', 'effectiveRate']
    ) ||
  ...
  const isQuantity =
    ...
    (isRecord(value.quantity) &&
      ...
      isCanonicalExactMoney({ amount: value.quantity.amount, currencyCode: 'AUD' }) &&
      isBoundedIdentity(value.quantity.unit));
  const isRate = (() => {
    if (value.effectiveRate === undefined) return true;
    if (
      !isRecord(value.quantity) ||
      ...
      !isCanonicalExactMoney({ amount: value.effectiveRate.amount, currencyCode: accountingCurrencyCode }) ||
      value.effectiveRate.unit !== value.quantity.unit
    ) {
      return false;
    }
    const quantity = parseCanonicalDecimal(String(value.quantity.amount));
    const rate = parseCanonicalDecimal(String(value.effectiveRate.amount));
    return (
      value.chargeClassification === 'usage' &&
      quantity.coefficient > 0n &&
      rate.coefficient >= 0n &&
      formatExactDecimalValue(multiplyExactDecimalValues(quantity, rate)) === value.amount
    );
  })();
  return isQuantity && isRate;
};

const isIdentityPreimage = (value: unknown): value is FinancialScopeBaselineIdentityPreimageV2 => {
  if (!isRecord(value) || !isCommonBaseline(value)) return false;
  ...
  const period = value.period as FinancialBaselinePeriodV2;
  if (period.observed === undefined || period.coverage.length === 0) return false;
  if (value.baselineKind === 'owner') {
    if (
      !OWNER_SCOPE_KINDS.has(String(value.scopeKind)) ||
      !hasExactFields(
        value,
        [...commonRequired, 'evidenceBundleId', 'accountingCurrency', 'chargeInclusionPolicyRef', 'components'],
        commonOptional
      ) ||
    ...
    const coverageIds = new Set((value.period as FinancialBaselinePeriodV2).coverage.map(item => item.coverageId));
    return (
      value.components.every(component =>
        isComponent(
          ...
          coverageIds,
          String((value.accountingCurrency as JsonRecord).currencyCode),
          value.estimateLens
        )
      ) && new Set(value.components.map(component => component.componentId)).size === value.components.length
    );
  }
  return (
...
export const canonicalizeFinancialScopeBaselineIdentityV2 = (value: FinancialScopeBaselineIdentityPreimageV2): string => {
  if (!isIdentityPreimage(value)) throw new TypeError('Invalid FinancialScopeBaselineIdentityPreimageV2.');
  return canonicalizeValidatedFinancialScopeBaselineIdentityV2(value);
};

const exactSum = (values: string[]): string => formatExactDecimalValue(sumCanonicalDecimals(values));

const isAvailableOwner = (value: JsonRecord): boolean => {
  if (
    !hasExactFields(
      value,
      [
    ...
    !isCanonicalExactMoney(value.total) ||
    !isRecord(value.accountingCurrency) ||
  ...
  if (!isIdentityPreimage(identity)) return false;
  const total = value.total as { amount: string; currencyCode: string };
  const reconciliation = value.reconciliation as {
    componentTotal: string;
    sourceTotal: string;
    withheldTotal: string;
    residualTotal: string;
  };
  const componentTotal = exactSum((value.components as FinancialOwnerBaselineComponentV2[]).map(component => component.amount));
  if (
    componentTotal !== total.amount ||
    componentTotal !== reconciliation.componentTotal ||
    ![reconciliation.sourceTotal, reconciliation.withheldTotal, reconciliation.residualTotal].every(amount =>
      isCanonicalExactMoney({ amount, currencyCode: total.currencyCode })
    ) ||
    exactSum([componentTotal, reconciliation.withheldTotal, reconciliation.residualTotal]) !== reconciliation.sourceTotal
  ) {
    return false;
  }
  return value.baselineId === `sha256:${sha256Utf8(canonicalizeValidatedFinancialScopeBaselineIdentityV2(identity))}`;
};

const isAvailableAggregate = (value: JsonRecord): boolean => {
  if (
    !hasExactFields(
      value,
      [
    ...
    value.reconciliation.status !== 'reconciled' ||
    value.reconciliation.difference !== '0' ||
    !isCanonicalExactMoney({ amount: value.reconciliation.memberTotal, currencyCode: value.total.currencyCode }) ||
    !isCanonicalExactMoney({ amount: value.reconciliation.residualTotal, currencyCode: value.total.currencyCode })
  ) {
    return false;
  }
  const total = value.total as { amount: string; currencyCode: string };
  const reconciliation = value.reconciliation as { memberTotal: string; residualTotal: string };
  if (exactSum([reconciliation.memberTotal, reconciliation.residualTotal]) !== total.amount) return false;
  const { status: _status, baselineId: _baselineId, total: _total, reconciliation: _reconciliation, ...identity } = value;
  return isIdentityPreimage(identity) && value.baselineId === `sha256:${sha256Utf8(canonicalizeValidatedFinancialScopeBaselineIdentityV2(identity))}`;
};

const isUnavailable = (value: JsonRecord): boolean =>
  hasExactFields(
    value,
    [
      'schemaVersion',
  ...
  [value.summary.requestedRoleCount, value.summary.producedRoleCount, value.summary.matchedRoleCount].every(
...
export const isFinancialScopeBaselineEnvelopeV2 = (value: unknown): value is FinancialScopeBaselineEnvelopeV2 =>
  isRecord(value) && isCommonBaseline(value) && (isAvailableOwner(value) || isAvailableAggregate(value) || isUnavailable(value));
