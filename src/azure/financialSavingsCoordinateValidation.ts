...
import type {
  ...
  FinancialSavingsCoordinateEnvelopeV1,
...
import {
  ...
  type FinancialSavingsJsonRecord,
...
export const createFinancialSavingsDenominatorIdV1 = (value: FinancialSavingsDenominatorIdentityPreimageV1): string =>
  `sha256:${sha256Utf8(canonicalizeFinancialSavingsDenominatorIdentityV1(value))}`;

export const canonicalizeFinancialSavingsActivationIdentityV1 = (value: FinancialSavingsActivationIdentityPreimageV1): string =>
  JSON.stringify(
    canonicalizeFinancialSavingsJsonValue({
...
export const createFinancialSavingsActivationIdV1 = (value: FinancialSavingsActivationIdentityPreimageV1): string =>
  `sha256:${sha256Utf8(canonicalizeFinancialSavingsActivationIdentityV1(value))}`;

export const canonicalizeFinancialSavingsAllocationIdentityV1 = (value: FinancialSavingsAllocationIdentityPreimageV1): string =>
  JSON.stringify(
    canonicalizeFinancialSavingsJsonValue({
...
export const createFinancialSavingsAllocationIdV1 = (value: FinancialSavingsAllocationIdentityPreimageV1): string =>
  `sha256:${sha256Utf8(canonicalizeFinancialSavingsAllocationIdentityV1(value))}`;

const decimalToMinorUnits = (value: string, scale: number): number | undefined => {
  ...
  if (discarded[0] !== undefined && discarded[0] >= '5') units += 1n;
  return units <= BigInt(Number.MAX_SAFE_INTEGER) ? Number(units) : undefined;
};

const isActivationDecision = (state: unknown, result: unknown, reason: unknown): boolean => {
...
const validateComposedCoordinate = (
  ...
  if (
    !hasExactFinancialSavingsFields(value as unknown as FinancialSavingsJsonRecord, [
      'status',
      'coordinateId',
    ...
    value.activations.length > 20_000 ||
    ...
    value.aggregate.allocationIds.length > 20_000 ||
  ...
  if (
    scenarioCoverageEvidence?.role !== 'recommendation-scenario-set' ||
  ...
  for (const allocation of value.allocations) {
    ...
    if (
      ...
      (allocation.eligibility.kind === 'mapped' &&
        (!eligibility ||
          ...
          !haveSameFinancialSavingsSet(
            allocation.eligibility.currentComponentIds,
            eligibility.currentBaselineMapping.mappings.map(mapping => mapping.currentComponentId)
          ) ||
    ...
    if (allocation.allocationId !== createFinancialSavingsAllocationIdV1(allocationIdentity)) return false;
    for (const componentId of allocation.billableComponentIds) {
      const componentKey = `${allocation.baselineId}\u0000${componentId}`;
      if (allocatedBaselineComponents.has(componentKey)) return false;
      allocatedBaselineComponents.add(componentKey);
    }
    allocationById.set(allocation.allocationId, allocation);
  }

  const contributedAllocationIds: string[] = [];
  const ownerScopeIds = new Set<string>();
  for (const contribution of value.resourceContributions) {
    ...
    if (
      allocations.some(allocation => !allocation || allocation.ownerScopeId !== contribution.ownerScopeId) ||
...
export const validateFinancialSavingsCoordinateEnvelopeV1 = (
  value: unknown,
  authorityCoordinate: FinancialAuthorityCoordinateV1,
  eligibilityById: Map<string, FinancialEligibilityAssessmentV1>,
  evidenceById: Map<string, FinancialEvidenceReferenceV1>,
  ...
  if (!isFinancialSavingsRecord(value) || value.coordinateId !== authorityCoordinate.coordinateId) return false;
  if (value.status === 'unavailable') {
    return (
      hasExactFinancialSavingsFields(value, ['status', 'coordinateId', 'unavailableReason'], ['currentAggregateBaselineId']) &&
